...
class Admin extends CI_Controller
{
    ...
    public function data_pengajar()
    {
        ...
        $data['user'] = $this->m_pengajar->tampil_data()->result();
        $this->load->view('admin/data_pengajar', $data);

    }

    public function detail_pengajar($nip)
    {
        $this->load->model('m_pengajar');
        $where = array('nip' => $nip);
        $detail = $this->m_pengajar->detail_pengajar($nip);
        $data['detail'] = $detail;
        $this->load->view('admin/detail_pengajar', $data);
    }

    public function update_pengajar($nip)
    {
        $this->load->model('m_pengajar');
        $where = array('nip' => $nip);
        $data['user'] = $this->m_pengajar->update_pengajar($where, 'pengajar')->result();
        $this->load->view('admin/update_pengajar', $data);
    }

    public function pengajar_edit()
    {
        $this->load->model('m_pengajar');
    ...
    public function delete_pengajar($nip)
    {
        $this->load->model('m_pengajar');
        $where = array('nip' => $nip);
        $this->m_pengajar->delete_pengajar($where, 'pengajar');
        $this->session->set_flashdata('user-delete', 'berhasil');
        redirect('admin/data_pengajar');
    }
    ...
    public function add_pengajar()
    {
        ...
        $this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email|is_unique[pengajar.email]', [
        ...
        if ($this->form_validation->run() == false) {
            $this->load->view('pengajar/registration');
        } else {

            $data = [
                'nip' => htmlspecialchars($this->input->post('nip', true)),
                'email' => htmlspecialchars($this->input->post('email', true)),
                'nama_pengajar' => htmlspecialchars($this->input->post('nama', true)),
            ...
            redirect(base_url('admin/data_pengajar'));
    ...
    public function tambah_materi()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                'nama_pengajar' => htmlspecialchars($this->input->post('nama_pengajar', true)),
